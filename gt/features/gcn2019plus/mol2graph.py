...
Chirality                          One-hot encoded {None, R, S, E}                                             4
total                                                                                                          41
...
the bond stereo is 'STEREONONE': 1 0 0 0
the bond stereo is 'STEREOANY': 0 1 0 0
the bond stereo is 'STEREOZ': 0 0 1 0
the bond stereo is 'STEREOE': 0 0 0 1

the bond is conjugated: 0 1
the bond direction is 'NONE': 1 0 0 0
the bond direction is 'BEGINWEDGE': 0 1 0 0
the bond direction is 'BEGINDASH': 0 0 1 0
the bond direction is 'UNKNOWN': 0 0 0 1
total: 13
...
def get_atom_features(atom=None):
    ...
    chirality_onehot = [int(i == chirality) for i in range(0, 4)]



    # Total
    total = [atomic_num] + atomic_num_onehot + [valence] + valence_onehot + [aromatic_onehot] + onehot_hybridization + \
            formal_charge_onehot + default_valence_onehot + rings + chirality_onehot
...
def get_bond_features(bond=None):
    ...
    bond_stereo_onehot = [int(i == bond_stereo) for i in range(0, 4)]

    # Bond conjugated
    bond_conjugated = bond.GetIsConjugated()
    bond_conjugated_onehot = int(bond_conjugated)


    # Total
    total = bond_type_onehot + bond_stereo_onehot + [bond_conjugated_onehot]
...
#    print(len(get_atom_features(atom)))
#for bond in mol.GetBonds():
#    print(get_bond_features(bond))
#    print(len(get_bond_features(bond)))
...
def mol2graph(mol):
    ...
    if len(mol.GetBonds()) > 0:  # mol has bonds
        ...
        edge_attr = np.empty((0, 9), dtype=np.int64)
