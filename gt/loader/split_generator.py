...
def prepare_splits(dataset):
    ...
    if mode == 'GTNMR-inference' and task_type == 'regression' and task == 'node':
        setup_infer_split(dataset)
    elif mode == 'GTNMR-custom-inference' and task_type == 'regression' and task == 'node':
        setup_infer_split(dataset)
    elif split_mode == 'standard':
        setup_standard_split(dataset)
    elif split_mode == 'random' and (task_type != 'regression' and task != 'node'):
        ...
        setup_random_split_node_regression(dataset)
...
def setup_random_split_node_regression(dataset):
    split_ratios = cfg.dataset.split
    formats = cfg.dataset.format
    pyg_dataset_id = formats.split('-', 1)[1]
    seed = cfg.seed

    if pyg_dataset_id == 'gt13C' or pyg_dataset_id == 'gt1H':
        train_index = [i for i, data in enumerate(dataset) if data.is_train]
        val_test_index = [i for i, data in enumerate(dataset) if not data.is_train]
        np.random.seed(seed)
        np.random.shuffle(val_test_index)
        num_val = int(len(val_test_index) * split_ratios[1] / (1 - split_ratios[0]))
        num_test = len(val_test_index) - num_val

        val_index = val_test_index[:num_val]
        test_index = val_test_index[num_val:num_val + num_test]
        set_node_regression_dataset_splits(dataset, [train_index, val_index, test_index])

    else:

        raise ValueError(f"Unsupported dataset format: {formats}")


def set_node_regression_dataset_splits(dataset, splits):
...
def setup_infer_split(dataset):
    """Set inference split for inference-only mode.

    Set the inference split to be the entire dataset.
    """
    task_level = cfg.dataset.task
    task_type = cfg.dataset.task_type
    if task_level == 'graph' or (task_level == 'node' and task_type == 'regression'):
        set_dataset_attr(dataset, 'infer_graph_index', np.arange(len(dataset)), len(dataset))
    else:
...
def setup_random_split(dataset):
    ...
    test_index = val_test_index[test_index]    # train_index, val_index, test_index is np.array，index，dataset.data.y index，split_ratios
